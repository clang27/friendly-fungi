...
public class TilesMapGenerator : MonoBehaviour {
    [Header("[Tiles prefabs]")] public List<GameObject> tiles;

    [Header("[Map Parameters]")] public GameObject map;
    ...
    [Header("[Map Filling]")] public GeneratorParameter additionalFilling;
    ...
    [Header("[Map roads between POI's]")] public EnableDisable roads;
    ...
    [Header("[Ladders on map]")] public EnableDisable ladders;
    ...
    private IEnumerator NewMap() {
        if (_scalerSystem) {
            ...
            while (!_scalerSystem.Reversed) yield return new WaitForSeconds(0.1f);
        }
        ...
        GenerateMap(map, mapSize, holesCount, holesSizes, heightsCount, heightsSizes, maxHeight,
            heightSmoothing, contentOnMap, poiCount, roads, roadsFilling, roadsFenceChance, roadsBetweenPOI, ladders,
            laddersChance);
    ...
    public void GenerateMap(GameObject _map, int _mapSize, GeneratorParameter _holesCount,
        GeneratorParameter _holesSizes, GeneratorParameter _heightsCount, GeneratorParameter _heightsSizes,
        GeneratorParameter _maxHeight, EnableDisable _heightSmoothing,
        EnableDisable _POIs, int _POIsCount, EnableDisable _roads, int _roadsFilling, int _roadsFenceChance,
        int _roadsBetweenPOI, EnableDisable _ladders,
        int _laddersChance) {
                ...
                for (var i = 0; i < pointsOfInterest.Count; i++) {
                    for (var a = i; a < pointsOfInterest.Count; a++) {
                        ...
                        if (createThisConnection) {
                            var minX = pointsOfInterest[i][0] < pointsOfInterest[a][0]
                                ? pointsOfInterest[i][0]
                                : pointsOfInterest[a][0];
                            var maxX = pointsOfInterest[i][0] < pointsOfInterest[a][0]
                                ? pointsOfInterest[a][0]
                                : pointsOfInterest[i][0];
                            var minZ = pointsOfInterest[i][1] < pointsOfInterest[a][1]
                                ? pointsOfInterest[i][1]
                                : pointsOfInterest[a][1];
                            var maxZ = pointsOfInterest[i][1] < pointsOfInterest[a][1]
                                ? pointsOfInterest[a][1]
                                : pointsOfInterest[i][1];
        ...
        if (_heightsCount != GeneratorParameter.None) {
            ...
            for (var i = 0; i < heightsCountToCreate; i++) {
                var hX = Random.Range(0, _xSize);
                var hZ = Random.Range(0, _zSize);
                heightMap = RaiseHeight(heightMap, hX, hZ, maxHeightSize, maxHeightInTiles, holesMap, 0,
                    _heightSmoothing);
            }
        }
        ...
        if (_heightSmoothing == EnableDisable.Enabled)
            for (var i = 0; i < _xSize; i++) {
                for (var a = 0; a < _zSize; a++) SmoothHeights(heightMap, i, a);
            }
        //-------------------------------------------------------------
        ...
        for (var i = 0; i < _xSize; i++) {
            for (var a = 0; a < _zSize; a++) roadsSumHeights += heightMap[i, a];
        }
        ...
        if (_POIs == EnableDisable.Enabled && roads == EnableDisable.Enabled) {
            foreach (var t in pointsOfInterest) {
                ...
                if (zPos - 1 > 0) heightMap[xPos, zPos - 1] = roadsHeight;
            }
        ...
        if (_ladders == EnableDisable.Enabled)
            for (var i = 0; i < _xSize; i++) {
                for (var a = 0; a < _zSize; a++) {
                    ...
                    var right = i + 1 < _xSize && !holesMap[i + 1, a] && !laddersMap[i + 1, a] &&
                                heightMap[a, i + 1] == myHeight + 1;
                    var left = i - 1 >= 0 && !holesMap[i - 1, a] && !laddersMap[i - 1, a] &&
                               heightMap[a, i - 1] == myHeight + 1;
                    var up = a + 1 < _zSize && !holesMap[i, a + 1] && !laddersMap[i, a + 1] &&
                             heightMap[a + 1, i] == myHeight + 1;
                    var down = a - 1 >= 0 && !holesMap[i, a - 1] && !laddersMap[i, a - 1] &&
                               heightMap[a - 1, i] == myHeight + 1;
                    ...
                    if (right && !left && !down && !up) //Ladder to right
                        if (i - 1 >= 0)
                            if (heightMap[a, i - 1] == myHeight)
                                if (!IsHole(i - 1, a, holesMap) && !IsHole(i + 1, a, holesMap) &&
                                    !IsHole(i, a, holesMap)) {
                                    y = 0f;
                                    needSpawn = true;
                                }
                    ...
                    if (!right && !left && down && !up) //Ladder to down
                        if (a + 1 < _zSize)
                            if (heightMap[a + 1, i] == myHeight)
                                if (!IsHole(i, a - 1, holesMap) && !IsHole(i, a + 1, holesMap) &&
                                    !IsHole(i, a, holesMap)) {
                                    y = 90f;
                                    needSpawn = true;
                                }
                    ...
                    if (needSpawn && Random.Range(0, 100) < _laddersChance) {
                        ...
                        ladder.transform.position = new Vector3(_mapPos.x + i * 2f, _mapPos.y + heightMap[a, i] * 2f,
                            _mapPos.z + a * 2f);
        ...
        for (var i = 0; i < _xSize; i++) {
            for (var a = 0; a < _zSize; a++) {
                if (!holesMap[i, a]) SpawnTile(i, a, x, z, _mapPos, _map.transform, roadsMap[a, i], heightMap[i, a]);
        ...
        if (_POIs == EnableDisable.Enabled)
            for (var i = 0; i < pointsOfInterest.Count; i++) {
                ...
                poiObj.transform.position = new Vector3(_mapPos.x + zPos * 2f, _mapPos.y + heightMap[xPos, zPos] * 2f,
                    _mapPos.z + xPos * 2f);
        ...
        if (_POIs == EnableDisable.Enabled && _roads == EnableDisable.Enabled) {
            ...
            for (var i = 0; i < _xSize; i++) {
                for (var a = 0; a < _zSize; a++) {
                    ...
                    //Additional Tiles
                    if ((left && down) || (right && down) || (right && up) || (up && left) ||
                        (up && down && right && left) ||
                        (up && down && right) || (up && left && right) || (up && left && down) ||
                        (right && left && down))
                        if (holesMap[a, i])
                            SpawnTile(i, a, i * 2, a * 2, _mapPos, _map.transform, true, heightMap[a, i]);

                    if (spawned) {
                        road.transform.position = new Vector3(_mapPos.x + i * 2f, _mapPos.y + heightMap[a, i] * 2f,
                            _mapPos.z + a * 2f);
    ...
    private void AdditionalFilling(GeneratorParameter _additionalFilling, int sizeOfMap) {
        ...
        if (_additionalFilling != GeneratorParameter.None) {
            ...
            for (var a = 0; a < countsCycle; a++) {
                var circleTreesPos =
                    new Vector3(Random.Range(0f, sizeOfMap * 2f), 15f, Random.Range(0f, sizeOfMap * 2f));
                ...
                for (var i = 0; i < objectsCounts; i++) {
                    ...
                    if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                        if (hit.transform.name.Contains("Tile") &&
                            IsPosAvailableByDistance(hit.point, treesPoints, 1.5f) &&
                            isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                            var tree = Instantiate(treesPrefabs[Random.Range(0, treesPrefabs.Count)], hit.point,
                                Quaternion.identity, lastMap.transform);
                            tree.transform.eulerAngles = new Vector3(Random.Range(-7.5f, 7.5f), Random.Range(0f, 360f),
                ...
                for (var i = 0; i < objectsCounts / 3; i++) {
                    ...
                    if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                        if (hit.transform.name.Contains("Tile") &&
                            ...
                            isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                            var tree = Instantiate(bushsPrefabs[Random.Range(0, bushsPrefabs.Count)], hit.point,
                                Quaternion.identity, lastMap.transform);
                ...
                for (var i = 0; i < objectsCounts * 4; i++) {
                    ...
                    if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                        if (hit.transform.name.Contains("Tile") &&
                            ...
                            isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                            var tree = Instantiate(grassPrefabs[Random.Range(0, grassPrefabs.Count)], hit.point,
                                Quaternion.identity, lastMap.transform);
                            tree.transform.eulerAngles = new Vector3(0f, Random.Range(0f, 360f), 0f);
                ...
                for (var i = 0; i < objectsCounts; i++) {
                    ...
                    if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                        if (hit.transform.name.Contains("Tile") &&
                            isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                            var tree = Instantiate(littleStonesPrefabs[Random.Range(0, littleStonesPrefabs.Count)],
                                hit.point, Quaternion.identity, lastMap.transform);
                            tree.transform.eulerAngles = new Vector3(Random.Range(0f, 360f), Random.Range(0f, 360f),
            ...
            for (var i = 0; i < objectsCounts * countsCycle * (int)_additionalFilling; i++) {
                ...
                if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                    if (hit.transform.name.Contains("Tile") && IsPosAvailableByDistance(hit.point, grassPoint, 0.25f) &&
                        isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                        var tree = Instantiate(grassPrefabs[Random.Range(0, grassPrefabs.Count)], hit.point,
                            Quaternion.identity, lastMap.transform);
                        tree.transform.eulerAngles = new Vector3(0f, Random.Range(0f, 360f), 0f);
            ...
            for (var i = 0; i < objectsCounts / 2; i++) {
                ...
                if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                    if (hit.transform.name.Contains("Tile") &&
                        ...
                        isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                        var tree = Instantiate(bigStonesPrefabs[Random.Range(0, bigStonesPrefabs.Count)], hit.point,
                            Quaternion.identity, lastMap.transform);
                        tree.transform.eulerAngles = new Vector3(Random.Range(0f, 360f), Random.Range(0f, 360f),
                            Random.Range(0f, 360f));
            ...
            for (var i = 0; i < objectsCounts / 2; i++) {
                ...
                if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                    if (hit.transform.name.Contains("Tile") &&
                        ...
                        isPosInRangeOf(hit.point, treesPoints, 8f) &&
                        isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                        var tree = Instantiate(branchsPrefabs[Random.Range(0, branchsPrefabs.Count)], hit.point,
                            Quaternion.identity, lastMap.transform);
                        tree.transform.eulerAngles = new Vector3(0f, Random.Range(0f, 360f), 0f);
            ...
            for (var i = 0; i < objectsCounts / 2; i++) {
                ...
                if (Physics.Raycast(rayPos, Vector3.down, out var hit, Mathf.Infinity))
                    if (hit.transform.name.Contains("Tile") &&
                        ...
                        isPosInRangeOf(hit.point, treesPoints, 8f) &&
                        isPosNotInPOI(hit.point, lastRoadsMap, lastLaddersMap)) {
                        var tree = Instantiate(logsPrefabs[Random.Range(0, logsPrefabs.Count)], hit.point,
                            Quaternion.identity, lastMap.transform);
                        tree.transform.eulerAngles = new Vector3(0f, Random.Range(0f, 360f), 0f);
    ...
    private void SpawnTile(int _i, int _a, float _x, float _z, Vector3 _mapPos, Transform _parent, bool _isRoad,
        int _height) {
    ...
    private int[,] RaiseHeight(int[,] currentMap, int _x, int _z, int maxSize, int maxHeight, bool[,] holesMap,
        int iteration, EnableDisable hs) {
        ...
        if (_z + 1 < currentMap.GetLength(1) &&
            Random.Range(0, 100) < chanceForAdditionalHeight * Random.Range(0.75f, 1f))
            currentMap = RaiseHeight(currentMap, _x, _z + 1, maxSize, maxHeight, holesMap, iteration, hs);
        ...
        if (_x + 1 < currentMap.GetLength(0) &&
            Random.Range(0, 100) < chanceForAdditionalHeight * Random.Range(0.75f, 1f))
            currentMap = RaiseHeight(currentMap, _x + 1, _z, maxSize, maxHeight, holesMap, iteration, hs);
        return currentMap;
    }
    ...
    private bool[,] CreateHoles(bool[,] currentMap, int x, int z, int maxSize, int iteration) {
        ...
        if (iteration < maxSize) {
            ...
            if (z + 1 < currentMap.GetLength(1) &&
                Random.Range(0, 100) < chanceForAdditionalHole * Random.Range(0.75f, 1f))
                currentMap = CreateHoles(currentMap, x, z + 1, maxSize, iteration);
    ...
    private bool IsNeighboringHole(int x, int z, bool[,] currentHoles) {
        ...
        if (z - 1 > 0)
            if (currentHoles[x, z - 1])
                isHole = true;
        if (z + 1 < currentHoles.GetLength(1))
            if (currentHoles[x, z + 1])
                isHole = true;
        if (x - 1 > 0)
            if (currentHoles[x - 1, z])
                isHole = true;
