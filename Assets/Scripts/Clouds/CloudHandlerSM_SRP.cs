    ...
    public class CloudHandlerSM_SRP : MonoBehaviour {
        ...
        private void Start() {
            if (Application.isPlaying) {
                if (rainbowMat != null) {
                    var RainbowC = rainbowMat.GetColor("_Color");
        ...
        private void UpdateCloudParams(bool Init, Material oceanMat) {
            ...
            if (windZone != null) {
                ...
                oceanMat.SetVector("_Velocity2", new Vector4(
                    Mathf.Lerp(_Velocity2.x, WindDir.x * WindParallaxFactor * 1,
                        0.0001f * Time_delta * 11 * WindParallaxFactor * 1),
                    Mathf.Lerp(_Velocity2.y, WindDir.y * WindParallaxFactor * 1,
                        0.0001f * Time_delta * 11 * WindParallaxFactor), 0.0f, CloudEvolution));
                //oceanMat.SetVector("_Velocity2", new Vector4() );
            }
            ...
            if (HasScatterShader) {
                ...
                //GRAB PARAMETERS
                var _SunColor = oceanMat.GetColor("_SunColor");
                var _ShadowColor = oceanMat.GetColor("_ShadowColor");
                //			float _ColorDiff = oceanMat.GetFloat("_ColorDiff");

                //					_CloudMap ("_CloudMap", 2D) = "white" {}
                //					_CloudMap1 ("_CloudMap1", 2D) = "white" {}

                var _Density = oceanMat.GetFloat("_Density");
                //if (AutoDensity) {

                //}
                oceanMat.SetFloat("_Density", Mathf.Lerp(_Density, CloudDensity, 0.27f * Time_delta + 0.05f)); //v3.4.3

                var _Coverage = oceanMat.GetFloat("_Coverage");
                var _Transparency = oceanMat.GetFloat("_Transparency");

                //	Vector4 _LightingControl = oceanMat.GetVector ("_LightingControl");   
                var _HorizonFactor = oceanMat.GetFloat("_HorizonFactor");

                var _Color = oceanMat.GetColor("_Color");
                var _FogColor = oceanMat.GetColor("_FogColor");
                    ...
                    if (is_after_17) {
                        ...
                        if (!is_after_222) {
                            if (cloudType == CloudPreset.Custom) { }
                                ...
                                oceanMat.SetColor("_ShadowColor",
                                ...
                                oceanMat.SetFloat("_Coverage",
                                    Mathf.Lerp(_Coverage, Coverage, Speed_factor * Time_delta));
                                oceanMat.SetFloat("_Transparency",
                ...
                oceanMat.SetFloat("_HorizonFactor",
