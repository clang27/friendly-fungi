...
public class MobileDemoInfiniCLOUD : MonoBehaviour {
    ...
    private void Update() {
        if (Camera.main != null && Camera.main.transform.position.y < toggleParticleRenderOrderY) {
            if (particlesRenderer.sortingOrder != -1) particlesRenderer.sortingOrder = -1;

            if (!smoothFadeOut) {
                if (upperCloudLayer.activeInHierarchy) upperCloudLayer.SetActive(false);
            }
            else {
                //upperCloudsMaterial.SetFloat("_Coverage", Mathf.Lerp(upperCloudsMaterial.GetFloat("_Coverage"), 0, Time.deltaTime * 2));
                upperCloudsMaterial.SetFloat("_Transparency",
                    Mathf.Lerp(upperCloudsMaterial.GetFloat("_Transparency"), maxTransp + transparencyOffset,
                        Time.deltaTime * 3));
                simpleCloudsMaterial.SetFloat("_CloudCover",
                    Mathf.Lerp(simpleCloudsMaterial.GetFloat("_CloudCover"), maxTranspA + transparencyOffset,
                        Time.deltaTime * 3));
            }
        }
        else {
            if (particlesRenderer.sortingOrder != 1) particlesRenderer.sortingOrder = 1;
            if (!smoothFadeOut) {
                if (!upperCloudLayer.activeInHierarchy) upperCloudLayer.SetActive(true);
            }
            else {
                // upperCloudsMaterial.SetFloat("_Coverage", Mathf.Lerp(upperCloudsMaterial.GetFloat("_Coverage"), 1.5f, Time.deltaTime * 2));
                upperCloudsMaterial.SetFloat("_Transparency",
                    Mathf.Lerp(upperCloudsMaterial.GetFloat("_Transparency"), minTransp + transparencyOffset,
                        Time.deltaTime * 3));
                simpleCloudsMaterial.SetFloat("_CloudCover",
                    Mathf.Lerp(simpleCloudsMaterial.GetFloat("_CloudCover"), minTranspA + transparencyOffset,
                        Time.deltaTime * 3));
            }
        }
    }
    ...
    private void OnGUI() {
        ...
        transparencyOffset =
            GUI.HorizontalSlider(new Rect(rightSide + 150 * 0, 125, 150, 40), transparencyOffset, -2, 2);
        ...
        var buttonText0 = "on";
        if (!cloudLayer0.activeInHierarchy) buttonText0 = "off";
        ...
        var buttonText1 = "on";
        if (!cloudLayerA.activeInHierarchy) buttonText1 = "off";
        ...
        var buttonText2 = "on";
        if (!cloudLayerB.activeInHierarchy) buttonText2 = "off";
        ...
        var buttonText3 = "on";
        if (!upperCloudLayer.activeInHierarchy) buttonText3 = "off";
