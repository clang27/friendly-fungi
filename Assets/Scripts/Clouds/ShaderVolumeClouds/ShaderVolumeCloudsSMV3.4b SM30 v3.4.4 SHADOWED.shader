...
Shader "SkyMaster/ShaderVolumeClouds-Desktop-SM3.0 v3.4.4 SHADOWED" {
    Properties {
		...
		[Enum(UnityEngine.Rendering.CullMode)] _CullMode("Cull Mode", Int) = 2
			...
			//_LightColor0("Sun Color", COLOR) = (1,1,1)
			shadowsLight("Light in shadows", float) = 0
    }
	SubShader{
		Tags {
...
{
	float3 objSpaceLightPos = mul(unity_WorldToObject, _WorldSpaceLightPos).xyz;
	#ifndef USING_LIGHT_MULTI_COMPILE
		return objSpaceLightPos.xyz - v.xyz * _WorldSpaceLightPos.w;
	#else
...
// *Legacy* Please use UnityWorldSpaceViewDir instead
  ...
  //float3 sunColor;
            ...
            VertexOutput vert (VertexInput v) {           
				...
				o.pos = vertexInput.positionCS;// float4(vertexInput.positionCS.xyz, 1000.0);
//#if UNITY_UV_STARTS_AT_TOP
				//o.pos.y *= -1;
//#endif							   				 
                //o.worldPos = mul(unity_ObjectToWorld, v.vertex);
				o.worldPos = vertexInput.positionWS.xyz;//v4.3


                //SCATTER
                //  TANGENT_SPACE_ROTATION;
                // o.ForwLight = mul(rotation,ObjSpaceLightDir(v.vertex)); //ObjSpaceLightDir(v.vertex);
                o.ForwLight =ObjSpaceLightDir(v.positionOS); //ObjSpaceLightDir(v.vertex);
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				float2 UVs1 = _Velocity1 * TimingF*_TimePass + UVs;
				...
				float2 UVs2 = (_Velocity2*TimingF*_TimePass + float2(_EdgeFactors.x, _EdgeFactors.y) + UVs);
				...
				float3 lightDirection = _WorldSpaceLightPos.xyz;///v4.3
				float3 lightColor = _SunColor;// sunColor; //_LightColor0.rgb; ///v4.3
				...
				float3 finalCol = diff * _SunColor.rgb* atten;//v4.3   //_LightColor0.rgb* atten;
				...
				float4 coordinates = TransformWorldToShadowCoord(i.worldPos);
				...
				if (attenuate < 0.1) {
					attenuate = attenuate + shadowsLight;
				}
				colorOUT = colorOUT * pow(directionalLight.color,1) * pow(attenuate,1);
...
//             	}
    ...
    //         	float Lerpfactor = (ColDiff+(_ShadowColor.a*(dot(lightDirect,normalN)-1)*ColDiff));
               ...
               // if(_Mode==0){
                ...
                clip(shaper*paintTexture1.a - _Cutoff+0.4);
                SHADOW_CASTER_FRAGMENT(i)
            }
            ENDCG
        }
