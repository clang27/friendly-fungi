...
namespace Artngame.SKYMASTER {
    public class ChainLightning_SKYMASTER : MonoBehaviour {
        ...
        [Range(1, 6)] public float Zigs_branching_divider = 2;
        ...
        //v1.7
        public Vector2 Light_delay = new(0.0f, 0.3f);
        public bool line_on;
        public Vector2 Line_delay = new(0.1f, 0.4f);

        public int Vertex_count = 30;
        public float Deviation = 10.2f;

        public Vector2 Line_width = new(10, 10); // width start end
        ...
        public List<LineRenderer> LineBranches = new();
        public List<LineRenderer> LineBranchesGlow = new();
        public int max_branches = 10;

        public bool SmoothLights; //lerp lights than enable/disable
        ...
        private readonly bool particle_on = true;
        ...
        private Vector3 Saved_start_pos; //v1.7
...
//			particles = GetComponent<ParticleEmitter>().particles;
        ...
        private void Update() {
            //v3.0
            if ((target == null) & (Random.Range(0, 2) == 1)) return;
            //
                ...
                if (Branches) {
                    if (LineBranches.Count < max_branches)
                        for (var i = 0; i < max_branches; i++) {
                            ...
                            var LinerTMP = Lineholder.AddComponent<LineRenderer>();
                    ...
                    if (line_glow_on && LineBranchesGlow.Count < max_branches)
                        for (var i = 0; i < max_branches; i++) {
                            var Glowholder = new GameObject();
                            Glowholder.name = "Glowholder" + i;
                            Glowholder.transform.parent = transform;
                            Glowholder.transform.localPosition = Vector3.zero;
                            var LinerGlow1 = Glowholder.AddComponent<LineRenderer>();
                ...
                if (is_parent | (!is_parent & Energized & (current_depth < max_depth))) {
                    ...
                    if (target1 != null)
                        if (target1.Length > 0) {
                            ...
                            if (target != null) {
                                var DIST_TARG = Vector3.Distance(Target_pos, target.position);
                            ...
                            if (target != null) {
                                var offset_bias1 = offset_bias;
                                if (offset_noise) {
                                    if (reset_noise) {
                                        offset_bias1.x = offset_bias.x +
                                                         Mathf.Clamp(0.1f * noise.Noise(timex + offset_bias.x), -0.15f,
                                                             0.15f);
                                ...
                                var Speedy = Downward_speed;
                                if (Slow_on_end) {
                                    var DIST_TARG1 = Vector3.Distance(Target_pos, target.position);
                                ...
                                if (Moving_Source)
                                    Target_pos = Vector3.Lerp(this_transform.position, target.position,
                                ...
                                if (line_on)
                                    if (Liner != null) {
                                        ...
                                        if (LinerGlow != null) LinerGlow.enabled = true; //v3.0
                                        Liner.enabled = true;
                                        if (Branches)
                                            for (var i = 0; i < LineBranches.Count; i++) {
                                                //LineBranches[i].SetWidth(0,0);
                                                //LineBranchesGlow[i].SetWidth(0,0);
                                                LineBranches[i].enabled = true;
                                                if (line_glow_on) LineBranchesGlow[i].enabled = true;
                                            }
                                        //}
                                        //}
                                        //}
                                        ...
                                        var Branch_L1_starts = new List<Vector3>();
                                        var Branch_L2_starts = new List<Vector3>();
                                        if (Branches) //Liner.SetVertexCount(Vertex_count*1); // double count for branches
                                            Liner.positionCount = Vertex_count; //v3.4.9
                                        else //Liner.SetVertexCount(Vertex_count);
                                            Liner.positionCount = Vertex_count; //v3.4.9

                                        var Current_line_pos = Saved_start_pos;
                                        var Current_end_pos = Vector3.zero;

                                        var Randomizer = (target.position - this_transform.position) / Vertex_count;
                                        Liner.SetPosition(0, this_transform.position);
                                        for (var i = 0; i < Vertex_count; i++) {
                                            Current_end_pos.x =
                                                Random.Range(Current_line_pos.x - Deviation + Randomizer.x,
                                                    Current_line_pos.x + Deviation + Randomizer.x);
                                            Current_end_pos.y = Current_line_pos.y + Randomizer.y;
                                            Current_end_pos.z =
                                                Random.Range(Current_line_pos.z - Deviation + Randomizer.z,
                                                    Current_line_pos.z + Deviation + Randomizer.z);
...
//											}
                                            ...
                                            //v2.2
                                            if (line_glow_on) LinerGlow.SetPosition(i, Current_end_pos);

                                            Current_line_pos = Current_end_pos;

                                            if ((i / 5 - Mathf.RoundToInt(i / 5) == 0) & (i > Vertex_count / 3))
                                                if (Branch_L1_starts.Count <
                                                    max_branches / 3) //GRAB Level 1 branches, using 1 third of availble
                                                    Branch_L1_starts.Add(Current_line_pos);
                                        }

                                        //Liner.SetPosition(Vertex_count-1,target.position); //v4.8.5
                                        Liner.SetPosition(Vertex_count - 1,
                                            new Vector3(Current_end_pos.x, target.position.y,
                                                Current_end_pos.z)); //v4.8.5

                                        if (Branches) {
                                            //v2.2 - Branches L1
                                            for (var j = 0; j < Branch_L1_starts.Count; j++) {
                                                var Current_line_pos1 = Branch_L1_starts[j];
                                                var Current_end_pos1 = Vector3.zero;
                                                ...
                                                for (var i = 0; i < Vertex_count; i = i + 1) {
                                                    Current_end_pos1.x = Random.Range(
                                                        Current_line_pos1.x - Deviation * 1 + Randomizer.x,
                                                        Current_line_pos1.x + Deviation * 2 + Randomizer.x);
                                                    Current_end_pos1.y = Current_line_pos1.y +
                                                                         Randomizer.y / Random.Range(1, 2);
                                                    Current_end_pos1.z = Random.Range(
                                                        Current_line_pos1.z - Deviation * 1 + Randomizer.z,
                                                        Current_line_pos1.z + Deviation * 2 + Randomizer.z);
                                                    ...
                                                    if ((i / 15 - Mathf.RoundToInt(i / 15) == 0) &
                                                        (i > Vertex_count / 3))
                                                        if (Branch_L2_starts.Count <
                                                            max_branches /
                                                            3) //GRAB Level 1 branches, using 1 third of availble
                                                            Branch_L2_starts.Add(Current_line_pos1);
                                                }
                                            }

                                            //v2.2 - Branches L2
                                            var L1_counter = Branch_L1_starts.Count;
                                            for (var j = 0; j < Branch_L2_starts.Count; j++) {
                                                var Current_line_pos1 = Branch_L2_starts[j];
                                                var Current_end_pos1 = Vector3.zero;
                                                ...
                                                if (line_glow_on) {
                                                    ...
                                                    LineBranchesGlow[j + L1_counter].positionCount =
                                                        Vertex_count; //v3.4.9
                                                    ...
                                                    LineBranchesGlow[j + L1_counter]
                                                        .SetPosition(0, Branch_L2_starts[j]);
                                                    ...
                                                    LineBranchesGlow[j + L1_counter].endColor =
                                                        new Color(0, 0, 0, 0); //v3.4.9
                                                }

                                                for (var i = 0; i < Vertex_count; i = i + 1) {
                                                    Current_end_pos1.x = Random.Range(11, 12) +
                                                                         Random.Range(
                                                                             Current_line_pos1.x - Deviation * 1 +
                                                                             Randomizer.x,
                                                                             Current_line_pos1.x + Deviation * 2 +
                                                                             Randomizer.x);
                                                    Current_end_pos1.y = Current_line_pos1.y +
                                                                         Randomizer.y / Random.Range(1, 2);
                                                    Current_end_pos1.z = Random.Range(
                                                    ...
                                                    if (line_glow_on)
                                                        LineBranchesGlow[j + L1_counter]
                                                            .SetPosition(i, Current_end_pos1);
                                                    Current_line_pos1 = Current_end_pos1;
...
//										}
                                ...
                                //v1.4
                                var next_in_Chain =
                                    target.gameObject.GetComponent(typeof(ChainLightning_SKYMASTER)) as
                                        ChainLightning_SKYMASTER;
                                //Energize target
                                if (next_in_Chain == null)
                                    next_in_Chain =
                                        target.gameObject.GetComponentInChildren(typeof(ChainLightning_SKYMASTER)) as
                                            ChainLightning_SKYMASTER;
                                    ...
                                    if (endLight) {
                                        if ((get_in == 1) & (target != null)) {
                                            if (!SmoothLights) {
                                                ...
                                                endLight.transform.position =
                                                    target.transform
                                                        .position; //particles[particles.Length - 1].position; //v3.4.5b
                                            }
                                            else {
                                                endLight.enabled = true;
                                                endLight.gameObject.SetActive(true);
                                                endLight.intensity = Mathf.Lerp(endLight.intensity,
                                                    max_end_light_intensity, Time.deltaTime * LightSpeed);
                                                //endLight.gameObject.SetActive(true);
                                                endLight.transform.position =
                                                    target.transform
                                                        .position; //particles[particles.Length - 1].position;//v3.4.5b
                                            ...
                                            if (!SmoothLights)
                                                ...
                                                endLight.intensity = Mathf.Lerp(endLight.intensity, 0,
                                                    Time.deltaTime * LightFadeSpeed * Random.Range(1, LightRipple));
                                        }
                                    }
                                    ...
                                    if (startLight) {
                                        if ((get_in == 1) & (target != null)) {
                                            if (!SmoothLights) {
                                                ...
                                                startLight.transform.position =
                                                    Saved_start_pos; // particles[0].position;//v3.4.5b
                                            }
                                            else {
                                                startLight.enabled = true;
                                                startLight.gameObject.SetActive(true);
                                                startLight.intensity = Mathf.Lerp(startLight.intensity,
                                                    max_start_light_intensity, Time.deltaTime * LightSpeed);
                                                startLight.transform.position =
                                                    Saved_start_pos; //particles[0].position;//v3.4.5b
                                            }
                                        }
                                            ...
                                            if (!SmoothLights)
                                                ...
                                                startLight.intensity = Mathf.Lerp(startLight.intensity, 0,
                                                    Time.deltaTime * LightFadeSpeed * Random.Range(1, LightRipple));
                                        }
                                    }
                                }
...
//								}
                                ...
                                if (line_on)
                                    if (Liner != null)
                                        if (Time.fixedTime - stop_time > Random.Range(Line_delay.x, Line_delay.y)) {
                                            //Liner.SetWidth(0,0);
                            ...
                            //v1.7
                            if (startLight & (target == null) &
                                (Time.fixedTime - stop_time > Random.Range(Light_delay.x, Light_delay.y))) {
                                if (!SmoothLights) {
                                    startLight.enabled = false;
                                    startLight.gameObject.SetActive(false);
                                }
