...
namespace Artngame.SKYMASTER {
    public class ParticleShadowsSM : MonoBehaviour {
        ...
        public Color ShadowColor = new(0.5f, 0.5f, 0.5f, 0.5f);
        ...
        private void Update() {
            ...
            for (var i = 0; i < ParticleList.Length; i++) {
                if (1 == 1 || counter == i) {
                    var Disp = ParticleList[i].position - Ref_light.position;
                    var ray = new Ray(Ref_light.position, Disp); //Camera.main.ScreenPointToRay(Input.mousePosition);
                    ...
                    if (Physics.Raycast(ray.origin, ray.direction, out hit, Shadow_max_dist)) {
                        if (affectAlpha) {
                            ...
                            ParticleList[i].startColor = new Color(ParticleList[i].startColor.r,
                                ParticleList[i].startColor.g, ParticleList[i].startColor.b, saveAlpha);
                        }
                        ...
                        if (Debug_on) Debug.DrawLine(ParticleList[i].position, hit.point, Color.red, 5);
                ...
                if (counter > ParticleList.Length) counter = 0;
            }
